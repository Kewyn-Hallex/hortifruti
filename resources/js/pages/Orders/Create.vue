...
const props = defineProps<{ fruits: Array<{ id: number; name: string; price: number; price_box: number; price_kg: number; price_bunch: number }> }>();
...
const items = ref<Item[]>([{ id: 1, productId: 0, qty: 1, price: 0, unit: 'box', kgPerBox: 20 }]);

const addItem = () => {
    const newId = Date.now();
    items.value.push({ id: newId, productId: 0, qty: 1, price: 0, unit: 'box', kgPerBox: 20 });
...
const handleFinalize = async () => {
    ...
    const payload = {
        ...
        items: items.value.map(i => ({
            ...
            unit: i.unit,
            qty: i.unit === 'box' ? Math.max(0, Math.floor(Number(i.qty || 0))) : Number(i.qty || 0),
    ...
    router.post('/pedidos', payload as any, {
        onStart: () => {
            submitting.value = true;
        },
        onSuccess: () => {
                ...
                <div class="space-y-4">
                    ...
                    <div v-for="(item, index) in items" :key="item.id"
                        ...
                        <div class="col-span-12 md:col-span-4 relative">
                            ...
                            <input v-model="productSearchQueries[item.id]" @input="onProductInput(item)"
                        ...
                        <div class="col-span-4 md:col-span-2">
                            ...
                            <input v-model.number="item.qty" :step="item.unit === 'box' ? 1 : 0.01"
                                :min="item.unit === 'box' ? 1 : 0.01" @change="normalizeQtyForUnit(item)" type="number"
